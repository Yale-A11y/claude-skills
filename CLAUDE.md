...
`.gitignore` currently only ignores `.DS_Store`. These runtime/generated artifacts are currently tracked and should generally be gitignored rather than committed: `*/.playwright-cli/` (page `.yml` + `console-*.log` run traces), `focus-visibility-audit/*.png` (screenshots from a run), and `accessibility-audit/SKILL.md.bak`.
