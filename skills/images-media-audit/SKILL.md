...
> **Signature for this category:** the media element's identity plus the rule — for `<img>`,
> its `src` with any query string stripped; for `<video>`, its `src`/`<source>`; for a
> standalone `<svg>`, its shape (icon class or first `<use href>`). The same logo missing
> `alt` on every page is **one** finding; the same rule failing on two different images is
> **two**. Background-image and icon-font findings key on the CSS `background-image` URL or
