...
> **Signature for this category:** the control's identity plus the rule — its `name`/`id`
> where stable, else its selector path within its form. A site-search input unlabeled in the
> header of every page is **one** finding. Beware ids that are generated per page load
> (`input-a7f3`): when an id looks generated, key on selector path plus control `type`
> instead, or the same control will fail to collapse.
